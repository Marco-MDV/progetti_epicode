...
const cards = (books) => {
    ...
    books.forEach(element => {
        ...
        moreInfo.classList.add('link-secondary','link-underline-opacity-0')
        moreInfo.text='more info'
        moreInfo.href=`/dettagli.html?asin=${element.asin}`
        ...
        const remouveElement = document.createElement('button')
        remouveElement.classList.add('m-1', 'buttonCard','remouveElement')
        remouveElement.textContent='delete element'
        ...
        containerButton.append(remouveElement, buttonWishlist, buttonBuy)

        cardBody.append(cardTitle, cardNameArtis, cardDurationSong, containerButton, moreInfo, badge)
...
const remouveElement = () =>{
    const cards  = document.querySelectorAll('.card')
    cards.forEach(card  =>{
        card.querySelector('.remouveElement').addEventListener('click',()=>{
            card.remove(card)
        })
    })
}
